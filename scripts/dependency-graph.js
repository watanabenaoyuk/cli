...
const mermaidAnnotations = [] // mermaid dependency annotations
const dependedBy = {}
  ...
  for (const [name, edge] of node.edgesOut) {
    if (isOurs(name)) {
      if (!dependedBy[node.packageName].has(edge.name)) {
        ...
        mermaidAnnotations.push(`  ${stripName(node.packageName)}-->${escapeName(edge.name)};`)
        if (edge.to) {
          iterate(edge.to.target)
        }
      }
    }
  }
